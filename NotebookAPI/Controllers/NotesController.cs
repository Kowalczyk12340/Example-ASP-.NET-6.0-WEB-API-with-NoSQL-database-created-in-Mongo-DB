...
namespace NotebookAPI.Controllers
{
  ...
  public class NotesController : Controller
  {
    ...
    [NoCache]
    [HttpGet(template: "{bodyText}/{updatedForm}/{headerSizeLimit}")]
    public async Task<IEnumerable<Note>> Get(string bodyText, DateTime updatedForm, long headerSizeLimit)
    {
      return await _noteRepository.GetNote(bodyText, updatedForm, headerSizeLimit) ?? new List<Note>();
    }
